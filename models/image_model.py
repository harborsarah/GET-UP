...
class VGGBlock(nn.Module):
    def __init__(self, in_channels, middle_channels, out_channels, activation='relu'):
        super().__init__()
        if activation == 'relu':
...
class bts_fuse(nn.Module):
    def __init__(self, params, feat_out_channels, feat_out_channels_rad, num_features=512):
        ...
        self.lpg8x8     = local_planar_guidance(params, 8)
        ...
        self.lpg4x4     = local_planar_guidance(params, 4)
        ...
        self.lpg2x2     = local_planar_guidance(params, 2)
        ...
        self.conv1      = torch.nn.Sequential(nn.Conv2d(num_features // 16 + 4, num_features // 16, 3, 1, 1, bias=False),
                                              nn.ELU())
        self.weight0    = torch.nn.Sequential(nn.Conv2d(params.radar_gcn_channel_out, num_features // 16, 1, 1, bias=False),
                                              nn.Sigmoid())
        self.project0   = torch.nn.Sequential(nn.Conv2d(params.radar_gcn_channel_out, num_features // 16, 1, 1, bias=False),
                                              nn.ReLU())
        self.get_depth  = torch.nn.Sequential(nn.Conv2d(num_features // 16, 1, 3, 1, 1, bias=False),
                                              nn.Sigmoid())

    def forward(self, img_features, rad_features, radar_fuse_features, focal):
        ...
        # print(skip0.shape, skip1.shape, skip2.shape, skip3.shape)
        dense_features = torch.nn.ReLU()(img_features[4])
        dense_features = dense_features + rad_weight5*rad_project5
        ...
        iconv5 = iconv5 + rad_weight4*rad_project4
        ...
        daspp_feat = daspp_feat + rad_weight3*rad_project3
        ...
        iconv3 = iconv3 + rad_weight2*rad_project2
        ...
        iconv2 = iconv2 + rad_weight1*rad_project1
        upconv1 = self.upconv1(iconv2)
        rad_weight0 = self.weight0(radar_fuse_features)
        rad_project0 = self.project0(radar_fuse_features)
        upconv1 = upconv1 + rad_weight0*rad_project0
